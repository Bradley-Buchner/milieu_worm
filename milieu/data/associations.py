...
def load_node_sets(file_path, id_subset=[], exclude_splits=[], gene_names_path=None):
    return load_diseases(file_path, id_subset, exclude_splits, gene_names_path)


def load_diseases(
    associations_path, diseases_subset=[], exclude_splits=[], gene_names_path=None
):
    ...
    with open(associations_path) as associations_file:
        ...
        for row in reader:
            ...
            entrez_ids = set(
                [int(a.strip()) for a in row["gene_entrez_ids"].split(",")]
            )

            total += len(entrez_ids)
            diseases[disease_id] = ProteinSet(
                disease_id, disease_name, entrez_ids, split
            )
...
def write_associations(diseases, associations_path, threshold=10):
    ...
    disease_list = [
        ...
        if len(disease.proteins) >= threshold
    ]

    with open(associations_path, "w") as associations_file:
        writer = csv.DictWriter(
            associations_file,
            fieldnames=["Disease ID", "Disease Name", "Associated Gene IDs"],
        )
        writer.writeheader()
...
def load_doid(diseases, hdo_path="data/raw/disease_ontology.obo"):
    """
...
def load_disease_classes(
    diseases, hdo_path="data/raw/disease_ontology.obo", level=2, min_size=10
):
    """
    ...
    for disease in diseases.values():
        ...
        for parent in obo[doid].get_all_parents():
            if obo[parent].level == level:
                disease.class_name = (
                    obo[parent].name.replace("disease of ", "").replace("disease", "")
                )
    ...
    print(
        "Classified {:.2f}% ({}/{}) of diseases".format(
            100.0 * num_classified / len(diseases), num_classified, len(diseases)
        )
    )
...
def output_diseases(diseases, output_path):
    ...
    df = pd.DataFrame(
        [
            {
                "name": disease.name,
                "class": "" if disease.class_name is None else disease.class_name,
...
def _build_disease_matrix(id_to_disease, protein_to_idx):
    """
    """
...
def remove_duplicate_diseases(diseases_path, network_path, threshold=0.9):
    ...
    disease_matrix, idx_to_disease = build_disease_matrix(
        id_to_disease, protein_to_node
    )
    ...
    for a, b in zip(*dupicates):
        ...
        print(
            "Found duplicate {}-{}".format(
                idx_to_disease[a].name, idx_to_disease[b].name
            )
        )
...
def split_diseases_cc(split_sizes, disease_path, network_path, threshold=0.3):
    ...
    diseases_dict = {
        did: disease
        for did, disease in diseases_dict.items()
